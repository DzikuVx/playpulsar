...
class psDebug {
	...
	/**
	 * Default timeout for socket error sending
	 * @var int
	 */
	static public $streamTimeout = 2;
	...
	static public function create() {
		...
		set_error_handler ( array ('psDebug', "errorHandler" ), $flags );
		set_exception_handler ( array ('psDebug', "exceptionHandler" ) );
     ...
     */
		if (self::$errorHoldsExecution) {
			throw new psDebugException ( $errorFullText, $errno );
		} else {
			/*
     * display error
     */
	...
	static protected function formatExceptionMessage(Exception $exception) {
		...
		if (get_class ( $exception ) != 'psDebugException') {
	...
	static public function send($text) {
		...
		$fp = fsockopen ( self::$senderConfig ['url'], self::$senderConfig ['port'], $errno, $errstr, self::$streamTimeout );
		
		if (! $fp) {
			return false;
		}

		stream_set_timeout($fp, self::$streamTimeout);
 ...
 * 
 */
class psDebugException extends Exception {
